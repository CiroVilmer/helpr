...
export function LoginForm() {
  ...
  const personaId = UUID_RE.test(rawPersonaId) ? rawPersonaId : "";
  const hasInvite = personaId !== "";

  // Con link de invitación arrancamos en modo "signup" — es el flujo que el admin compartió.
  const [mode, setMode] = useState<Mode>(hasInvite ? "signup" : "signin");
  ...
  // Si el query string aparece más tarde (navegación client-side), saltamos a signup. Pattern
  // recomendado en React 19: comparar el prop con la última versión guardada y actualizar el
  // estado durante el render (no en useEffect).
  const [lastInvite, setLastInvite] = useState(hasInvite);
  if (hasInvite !== lastInvite) {
    setLastInvite(hasInvite);
    ...
    <div className="grid min-h-dvh lg:grid-cols-2">
      ...
      <main className="flex flex-col items-center justify-center bg-crema-base px-5 py-12">
        <div className="w-full max-w-sm">
          ...
          {hasInvite && (
            <p className="mt-4 flex items-start gap-2 rounded-lg bg-lima-suave px-3 py-2.5 text-sm text-[#2c4a0a]">
              <LinkIcon
                className="mt-0.5 size-4 shrink-0"
                aria-hidden="true"
              />
              Tenés una invitación. Tu cuenta queda vinculada a la persona del
              link al entrar.
            </p>
          )}
